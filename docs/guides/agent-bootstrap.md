...
3. `docs/plan/implementation_plan.md` §§3-4,15,17 + `docs/plan/decisions.md` ADR-001..045 + `specs/README.md` mapa Fase 0 (16, ADR-011) + Fase 1 (8, ADR-040) — fonte da ordem de execução, não o plano.
...
```bash
cat specs/README.md | grep -E "^\| \`0"           # mapa das fases, ✅ vs ⏳
git log --oneline -3                               # último feat (ex: 62d2a91 008)
for f in scripts/verify/f0-*.sh; do "$f" --quiet || exit 1; done
for f in scripts/verify/f[1-9]-*.sh; do "$f" --quiet || exit 1; done   # ADR-043: sem este laço a fase corrente fica sem portão
echo "harness $(ls scripts/verify/f[0-9]-*.sh | wc -l) oráculos"
...
for f in scripts/verify/f[1-9]-*.sh; do "$f" --quiet || exit 1; done   # idem (ADR-043)
...
→ TESTS 🔴 (f<fase>-NNN-*.sh 9-17 + evidence/red.txt) → IMPLEMENT 🟢 → CONVERGE (tasks.md 0×[ ] + harness)
```
...
- **Não hard-coda** contagem nem nome de item — o texto manda executar `cat specs/README.md | grep`, `git log --oneline -3` e os dois laços do harness. Quando `009` fechar, o próximo agente já vê `9/16` sem você ter editado este arquivo.
...
## Por que os dois laços (ADR-043)

Este arquivo enumerava só `f0-*`. Foi assim que `f1-017` convergiu e chegou a
`main` sem que CI, `pre-push` ou a promoção a pytest jamais o executassem: o
agente seguia a instrução ao pé da letra e via verde sobre 16 de 17 oráculos.
Quem enumera oráculos enumera as duas formas — o literal `f0-*` (congelado por
oito asserções em sete oráculos convergidos) e `f[1-9]-*`.
