...
- [x] T035 Atualizar `specs/README.md` índice — `007 | 0.3 | MyPy 2.3.1 strict | ✅ concluída (`a60c5b4`)` (FR-015 inquebrável com hash) — conforme `plan.md` Fase C:5 e ADR-011
- [x] T036 Executar `quickstart.md` validação completa em um comando (Cenário 1–6): `uv run mypy --version | grep -q "2.3.1" && uv run mypy --strict . --no-error-summary >/dev/null 2>&1 && sha256sum -c scripts/verify/manifest.sha256 && for f in scripts/verify/f0-*.sh; do "$f" --quiet || exit 1; done && ! test -f mypy.ini && grep -iq "007.*mypy.*✅.*a60c5b4" specs/README.md && git ls-files --error-unmatch specs/007-mypy/spec.md` → todos `OK` = `contracts/mypy-contract.md` satisfeito (SC-001..008)
