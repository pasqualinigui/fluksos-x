...
| FR-009 | profile `llm`: web+worker `4.30.0`, CH `25.12`, MinIO pinado, `TELEMETRY_ENABLED=false`; DATABASE_URL no PG compartilhado; init `01-users-dbs.sh` presente; prova de migração live = T026 🧑 (oráculo não sobe container) |
...
| FR-012 | init `01-users-dbs.sh` com bancos `fkx`+`langfuse` e usuários sem SUPERUSER |
