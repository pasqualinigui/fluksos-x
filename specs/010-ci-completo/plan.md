...
010 é dona designada de `.github/` (Emenda 1): estender `ci.yml` é uso previsto, não conflito. **Conflito se e somente se**: renomear/remover job `verify` (FR-009/003) ou mudar gatilhos/branches de forma que FRs da 003 reprovem — proibido sem ADR prévia. **Conflito adicional I (achado do vermelho 010, ADR-021)**: `f0-009` FR-014 conta linhas do manifest com igualdade (`==9`) e reprova com a 10ª linha legítima da 010 — mesma classe A1/A2 (asserção temporal); correção pré-autorizada (piso `>=9`, espelho do piso `>=5` da 005), aplicada só na Fase C. **Conflito adicional II (achado do verde 010, ADR-022)**: FR-004/005 da 003 exigem `@v7` literal (SHA autorizado como equivalente, SHA ⊃ tag) e FR-006 da 003 proíbe o vocabulário da 010 (reduzida aos vetores `pull_request_target|workflow_run`, dono atual: 010). Nenhum outro oráculo 001–009 é tocado pela 010; se a implementação descobrir necessidade, volta ao PLAN + ADR prévia (nunca fix direto — A1 não se repete).
