...
if [ -d "$ROOT/packages/cli" ] && [ ! -f "$ROOT/packages/cli/pyproject.toml" ]; then FR15_OK=0; EVID15="${EVID15}packages/ com cli/ sem pyproject.toml de membro (ADR-026); "; fi
if [ -n "$(ls "$ROOT/packages" 2>/dev/null | grep -v -x -e "core" -e "cli" | tr -d '[:space:]' || true)" ]; then FR15_OK=0; EVID15="${EVID15}packages/ com conteudo alem de core/+cli/ (ADR-023/026); "; fi
